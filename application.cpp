...
int Application::initAllegro5(){
	...
	// todo turned off fullscreen for debugging, feel free to put it back on...
...
//  al_set_new_display_flags(ALLEGRO_FULLSCREEN);                    // fullscreen
	...
	al_init_primitives_addon();                                      // why?
	timer = al_create_timer(1.0 / FPS);										// how often channels sent to csound + overhead
  if (!timer){
  ...
  //create metronome
	metronome = al_create_timer(BPM_TO_8BARS(BPM));
  if (!metronome){
	...
	messAround();																					// todo does this fix a message pass before init bug?
	...
	if(num_touches == 2) {
		for(int i = 0; i < 3; i++)	{ //check all fingers
			...
			if((fingers[i].x < CENTERX+100 && fingers[i].x > CENTERX-100) && (fingers[i].y < CENTERY+100 && fingers[i].y > CENTERY-100)){
				...
				angle1 = acos((fingers[i+1%3].x-CENTERX)/(sqrt(pow(fingers[i+1%3].x-CENTERX, 2) + pow(fingers[i+1%3].y-CENTERY, 2))))*(180.0/PI);
				angle2 = acos((fingers[i+2%3].x-CENTERX)/(sqrt(pow(fingers[i+2%3].x-CENTERX, 2) + pow(fingers[i+2%3].y-CENTERY, 2))))*(180.0/PI);
		...
		if(no_center){
			...
			if(abs(angle1-angle2)<GEST_ANGLE_SENS && abs(radius1-radius2)>GEST_RADII_SENS)
  			...
  			case ALLEGRO_EVENT_TOUCH_END: {
  				...
  				if (i >= 0 && i < num_touches) {
  					...
  					accept_more_touches = false; 							// this is reset by a timer event later in this function
						...
						///ssbtsound.setCsWaitState = false; 	// cswaitstate changes every time accept_more_touches...for readability
  				}
  					break;
  			}
			...
			case ALLEGRO_EVENT_TIMER: {
				if (event.timer.source == metronome){       // poll for which timer it is with if statements
					...
					switch(currentGesture){
						case TAP_GESTURE:{
							cout << "GESTURE = tap.\n";
							//User1.teleport(ssbtsound, AllRabbits,touches[0].x,touches[0].y);	// set values but dont flip yet
